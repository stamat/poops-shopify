...
export default function registerShopifyTags(engine, shopifyEngine) {
  engine.registerTag('section', {
    ...
    * render(ctx) {
      const name = yield this.liquid._evalValue(this.value, ctx)
  ...
  engine.registerTag('form', {
    ...
    * render(ctx) {
      ...
      const type = yield this.liquid._evalValue(parts[0], ctx)
      ...
      for (const part of parts.slice(1)) {
        ...
        if (!m) continue
        const val = yield this.liquid._evalValue(m[2], ctx)
  ...
  engine.registerTag('paginate', {
    ...
    * render(ctx) {
      const items = (yield this.liquid._evalValue(this.collectionExpr, ctx)) || []
      const pageSize = Number(yield this.liquid._evalValue(this.sizeExpr, ctx)) || 20
