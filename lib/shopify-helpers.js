...
export function formatMoney(cents, format = '${{ amount }}') {
  ...
  return format
    ...
    .replace(/\{\{\s*amount_no_decimals\s*\}\}/g, String(Math.round(amount)).replace(/\B(?=(\d{3})+(?!\d))/g, ','))
...
// Blog/order timestamps carry the shop's UTC offset ("2022-01-18T17:24:12-07:00").
// Formatting through a Date's local getters shifts the wall-clock day on a server
// in another timezone (Belgrade renders that -07:00 "Jan 18" as "Jan 19"). The ISO
// string already IS the shop's wall clock, so read its literal fields and rebuild a
// local Date — offset-independent, matching what Shopify prints. Non-ISO inputs
// ('now'/'today', unix seconds, Date) fall back to toDate, where local is correct.
