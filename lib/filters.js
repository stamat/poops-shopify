...
import { translate, formatMoney, handleize, imageSrc, toDisplayDate, strftime, modifyFont } from './shopify-helpers.js'
...
export default function registerShopifyFilters(engine, shopifyEngine) {
  ...
  engine.registerFilter('json', (obj) => JSON.stringify(obj ?? null)) // Shopify prints null for nil
  ...
  // No real checkout in preview, but the Buy-it-now slot should look real —
  // emit Shopify's unbranded dynamic checkout markup (Dawn styles these classes
  // in base.css). Disabled when the page's product is sold out, like the
  // add-to-cart button next to it. Installment terms still render nothing.
  engine.registerFilter('payment_button', function() {
    const product = this.context.getSync(['product']) || {}
    const variant = product.selected_or_first_available_variant
    const soldOut = variant ? variant.available === false : product.available === false
    return `<div data-shopify="payment-button" class="shopify-payment-button"><button type="button" class="shopify-payment-button__button shopify-payment-button__button--unbranded"${soldOut ? ' disabled' : ''}>Buy it now</button></div>`
  })
  ...
  engine.registerFilter('date', (input, fmt) => strftime(toDisplayDate(input), fmt || '%Y-%m-%d'))

  // Shopify's time_tag wraps a timestamp in <time datetime="ISO">…</time>, formatting
  // with a named date_format (Dawn's blog/account/order templates pass format: 'date'
  // or 'date_at_time') or a bare strftime string. Without it strictFilters:false lets
  // the raw ISO timestamp print. Named formats mirror Shopify's platform defaults; the
  // datetime attr keeps the original offset-bearing string.
  const DATE_FORMATS = {
  ...
  engine.registerFilter('time_tag', (input, ...args) => {
    const kw = kwargs(args)
    const named = kw.format || args.find(a => typeof a === 'string')
    const fmt = DATE_FORMATS[named] || named || DATE_FORMATS.date
    const date = toDisplayDate(input)
    if (isNaN(date.getTime())) return ''
    // keep the original offset-bearing string; 'now'/'today' aren't valid datetimes
    const iso = typeof input === 'string' && /^\d{4}-\d{2}-\d{2}/.test(input) ? input : date.toISOString()
    return `<time datetime="${iso}">${strftime(date, fmt)}</time>`
  })
  ...
  // mtime-keyed so watch-mode edits to inlined assets show up on the next
  // render without clearing: a statSync per use instead of a re-read per page.
  const assetCache = shopifyEngine.assetCache = new Map()
  engine.registerFilter('inline_asset_content', (file) => {
    const name = String(file)
    const fullPath = path.join(shopifyEngine.themeDir, 'assets', name)
    let mtimeMs
    try {
    ...
    } catch { /* race: deleted between stat and read */ }
    assetCache.set(name, { mtimeMs, content })
    return content
  })
